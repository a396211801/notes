...
abstract class medoo
{
    ...
    public function __construct($options = null)
    {
        try {
            ...
            if (is_array($options)) {
                ...
                $c = Yaf_Application::app()->getConfig();
                $config = $c->database->params->master->toArray();
    ...
    public function selects( $columns, $where = null)
    {
        $table = $this->getTable();
        $query = $this->query( 'SELECT ' . (
            is_array( $columns ) ? implode( ', ', $columns ) : $columns
            ) . ' FROM ' . $this->table_quote($table) . $this->where_clause( $where ) );
