...
(function (k) {
    var i = {
        put: function (a, b) {
            ...
            var b = ((a || window).location.hash).replace(/^#/, "");
            try {
                return k.browser.mozilla ? b : decodeURIComponent(b)
            } catch (c) {
                return b
            }
    ...
    var l = {
        id: "__jQuery_history", init: function () {
            ...
            return k("#" + this.id)[0].contentWindow.document
        }, put: function (a) {
            var b = this._document();
            b.open();
            b.close();
            i.put(a, b)
        }, get: function () {
            return i.get(this._document())
        }
    };

    function j(b) {
        b = k.extend({unescape: false}, b || {});
        i.encoder = a(b.unescape);
        ...
        function a(d) {
            ...
            if (typeof d == "string" && (d = c(d.split(""))) || typeof d == "function") {
                return function (e) {
        ...
        function c(d) {
            var e = new RegExp(k.map(d, encodeURIComponent).join("|"), "ig");
            return function (f) {
                return f.replace(e, decodeURIComponent)
            }
        }
    }
    ...
    g.timer = {
        _appState: undefined, _init: function () {
            var a = i.get();
            h._appState = a;
            h.callback(a);
            setInterval(h.check, 100)
        }, check: function () {
            var a = i.get();
            if (a != h._appState) {
                h._appState = a;
    ...
    g.hashchangeEvent = {
        _init: function () {
            h.callback(i.get());
            k(window).bind("hashchange", h.check)
        }, check: function () {
            h.callback(i.get())
        }, load: function (a) {
            i.put(a)
    ...
    var h = k.extend({}, g.base);
    if (k.browser.msie && (k.browser.version < 8 || document.documentMode < 8)) {
        h.type = "iframeTimer"
    } else {
        if ("onhashchange" in window) {
            h.type = "hashchangeEvent"
        } else {
            h.type = "timer"
        }
    }
